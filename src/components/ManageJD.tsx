...
import {
    ACCESS_PERMISSION,
    PLACEMENT_CYCLE_STATUS,
    PLACEMENT_CYCLE_TYPE,
} from "@prisma/client";
...
export default function ManageJDList() {
    ...
    const refreshData = useCallback(async () => {
        ...
        setIsRefreshing(true);
        try {
            const [jds, cycles] = await Promise.all([fetchAllJDs(), fetchAllCycles()]);
            setJDList(jds);
    ...
    const filteredJD = useMemo(() => {
        const filtered = jdList.filter((jd) => {
            ...
            const matchesCycle =
                selectedCycle === -1 || jd.placement_cycle_id === Number(selectedCycle);
            const matchesStatus =
                selectedStatus === "" ||
    ...
    return (
        <div className="p-4 sm:p-6 md:p-8 bg-gray-100 min-h-full">
            ...
            <div className="mt-4 flex flex-wrap items-center gap-3">
                <div className="flex flex-wrap items-center gap-2 flex-1">
                    ...
                    <select
                        ...
                        className="px-2 py-2 border border-gray-300 rounded-md text-sm text-gray-700 
                        bg-white shadow-sm focus:outline-none focus:ring-2 focus:ring-cyan-400 w-full sm:w-auto"
                    >
            ...
            {showLoadingScreen && (
                <div
                    className="fixed inset-0 z-50 bg-gradient-to-br from-[#0d1b24]/95 to-[#0a141d]/95 backdrop-blur-sm flex flex-col items-center justify-center"
                    role="status"
                    aria-live="polite"
                >
                    <div className="relative mb-6">
                        ...
                        <div className="h-16 w-16 rounded-full border-4 border-cyan-400/70 border-t-transparent motion-safe:animate-spin" />
                    </div>

                    <p className="text-cyan-200 text-lg font-medium tracking-wide">
                        Uploading JD Entry ...
                    </p>
                    <p className="mt-1 text-cyan-300/70 text-sm">
