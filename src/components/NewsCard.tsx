...
import {  NEWS_SUBDOMAIN_TAG } from "@prisma/client";
...
export default function NewsCard({ news, fetchNews, search,
    ...
    const [previewImageUrl, setPreviewImageUrl] = useState<string | null>(news.path_to_image);
    ...
    useEffect(() => {
        if (editMode) {
            ...
            if (!imageFormData && news.path_to_image) {
                setPreviewImageUrl(`${news.path_to_image}?t=${new Date().getTime()}`);
            }
        } else {
    ...
    const handleSave = async () => {
        try {
            ...
            formData.append("file", news.path_to_image);
        ...
        >
                                            ...
                                            <CompanySearchDropdown
                                                onSelect={(company) => {
                                                    if (!companyList.some((c: any) => c.company.id === company.id)) {
                                                        ...
                                                        toast.success(`Added ${company.company_name}`);
                                                    } else {
            ...
            {news.path_to_image && (
                <div className="relative w-full max-h-60 rounded-md overflow-hidden">
                    ...
                    >
                        <img
                            src={previewImageUrl || `${news.path_to_image}?t=${new Date().getTime()}`}
                            alt="News Visual"
            ...
            {/* Verified + Author */}
            <div className="flex justify-between items-center text-sm mt-2 border-t pt-2 border-gray-200">
                {editMode ? (
                    <select
                    ...
                    >
                        ...
                        <div className="flex justify-center gap-4 mt-4">
                            <button
                                onClick={async () => {
                                    try {
                                        await axios.delete("/api/news", { params: { id: news.id } });
