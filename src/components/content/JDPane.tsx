...
const JDPane: React.FC<JDPaneProps> = ({ props }) => {
    ...
    return (
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-5 p-4">
            {sorted.map((jd, idx) => {
                const ext = jd.jd_pdf_name?.split(".").pop()?.toLowerCase() || "pdf";
                const isCurrent = !!jd.is_current;

                const cardBase =
                    "relative rounded-xl transition-all duration-200 backdrop-blur-md overflow-hidden " +
                    "border hover:-translate-y-0.5 will-change-transform";
                const currentSurface =
                    "bg-gradient-to-br from-[#091520] via-[#0b1d2a] to-[#0f2230] border-cyan-500/50";
                const oldSurface =
                    "bg-gradient-to-br from-[#0b1118] via-[#0c121a] to-[#0d141d] border-cyan-900/30 hover:border-cyan-700/40";
                    ...
                    >
                        ...
                        <div className="p-5">
                            ...
                            <div className="flex items-start justify-between mb-2">
                                <h3 className="text-base font-semibold text-white truncate pr-8">
                                    {jd.company}
                                </h3>
                            </div>

                            {/* Role + Domains on SAME ROW */}
                                        ...
                                        return (
                                            <span
                                                ...
                                                className={cn(
                                                    color.bg,
                                                    color.text,
                                                    "px-2 py-0.5 rounded-full text-[11px] leading-tight"
                                                )}
                                            >
                                                {domain}
                                            </span>
                            ...
                            {/* Footer: cycle left, FILE TYPE bottom-right */}
                                ...
                                <span
                                    className={cn(
                                        ...
                                        isCurrent
                                            ...
                                            : "bg-[#112531] text-cyan-200 border border-cyan-800/40"
                                    )}
                                >
                            ...
                            <button
                                ...
                                className={cn(
                                    ...
                                    isCurrent
                                        ? "border-cyan-500/60 bg-cyan-900 text-white hover:bg-cyan-800 focus-visible:ring-cyan-500"
                                        : "border-cyan-800/40 bg-[#0d1b24] text-white hover:bg-[#10202b] focus-visible:ring-cyan-600"
                                )}
